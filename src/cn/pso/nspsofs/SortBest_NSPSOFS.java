...
public class SortBest_NSPSOFS {
	public void getNSResult(String dataSetName,List<Particle_NSPSOFS> list,String type) throws IOException {
		...
		List<Particle_NSPSOFS> nonDomS = new ArrayList<Particle_NSPSOFS>();
	   	 for (int i = 0; i < list.size();i++){
	     	int index = -1; 
		...
		for(Particle_NSPSOFS p:nonDomS){
...
//			System.out.println(p.pbest_pbest_fitness[0]+"------"+p.pbest_pbest_fitness[1]);
		...
		pw.close();
		System.out.println(dataSetName+":NSdone!");
